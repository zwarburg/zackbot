...
class Country
  ...
  # ISOLATE_COUNTRY = /\|[^\n\{\[]*(.*?)(?=\|\|)/
  ISOLATE_COUNTRY = /\|[^\{\[]*(.*?)(?=(?:\|\||\n))/
  PLAIN_TEXT_COUNTRY = /\|\s*([A-Za-z]+)\s*\|/
  
  def initialize(row)
    #TODO: add a check for this being a total row, if it is then skip it
  ...
  def parse_name!
    ...
    if raw_name.match?(IOC_TEMPLATE)
  ...
  def to_template(template, event)
    ...
    raise InvalidIoc.new(ioc) unless ioc.match?(/^[A-Z]{3}$/)
    ...
    return '' if ioc.nil?
    result = ''
    result += " | gold_#{@ioc} = #{@gold} | silver_#{@ioc} = #{@silver} | bronze_#{@ioc} = #{@bronze}"
    result += " | host_#{@ioc} = yes " if @host
    result += " | name_#{@ioc} = #{@name_param}" unless @name_param.empty?
